...
END {
    ...
    print search(queue, 1, next_right_blizzards, next_down_blizzards, next_left_blizzards, next_up_blizzards)
}

function search(queue, time, right_blizzards, down_blizzards, left_blizzards, up_blizzards,    next_queue, next_right_blizzards, next_down_blizzards, next_left_blizzards, next_up_blizzards) {
	...
	for (dir = 0 ; dir < n_dirs ; dir++){
		...
		if (new_row == rows - 1 && new_col == end_col)
		    return time + 1
...
function can_move(row,col,right_blizzards, down_blizzards, left_blizzards, up_blizzards,    pos){
	...
	!(pos in up_blizzards) &&	\
	(col >= 0) && (col < cols) &&	\
	(row < rows) && (row >= 0 || (row == -1 && col == start_col))
}
